...
func listen(listener *net.Listener, name string, logger *common.Logger) {
	for {
		...
		logger.WriteToFileOK(fmt.Sprintf("Accepted connection coming from RemoteAddress = %s and (self) LocalAddress  = %s", conn.RemoteAddr().String(), conn.LocalAddr().String()))
		buf := make([]byte, 1024)
		n, err := conn.Read(buf)
		if err != nil {
...
*/
func speak(self string, logger *common.Logger) {
	...
	for {
		...
		for i := 0; i < numberOfFriends; i++ {
			if !communicated[i] {
				communicated[i] = tryCommunicate(self, friends[i], logger)
			}
		}
	}
}
...
func register(address string, logger *common.Logger) (string, error) {
	for {
		...
		} else if result == common.MsgNameExists {
